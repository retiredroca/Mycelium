...
use sov_p2p::{SovereignNode, P2pConfig, NodeCapability, DiscoveryConfig};
use sov_post::Post;
use sov_storage::LocalStorage;
use sov_token::{Wallet, Tokenomics, RewardPool};
use tracing::{info, error, Level};
use tracing_subscriber::FmtSubscriber;
use libp2p::identity::Keypair;

#[derive(Parser)]
#[command(name = "sovereign-node")]
#[command(about = "The Sovereign Node Protocol - Decentralized Social Network", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
async fn start_node(listen: String, bootstrap: Option<String>) -> Result<()> {
    info!("Starting Sovereign Node Protocol...");
    ...
    let node = SovereignNode::new(config).await?;
...
async fn handle_network(action: NetworkCommands) -> Result<()> {
    match action {
        ...
        NetworkCommands::Info => {
            ...
            println!("╠════════════════════════════════════════════════════════════╣");
            println!("║ Protocol:           Sovereign Node Protocol v{}", env!("CARGO_PKG_VERSION"));
